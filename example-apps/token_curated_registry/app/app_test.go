...
func TestChallengeWonFlow(t *testing.T) {
	...
	require.Equal(t, "Unique registry listing 1", rapp.ballotKeeper.GetBallot(ctx, "Unique registry listing 1").Identifier, "Candidate 1 not added to registry after beating challenge")
...
func TestChallengeLostFlow(t *testing.T) {
	...
	ctx = rapp.NewContext(false, header)
	// Candidate 1 passed the challenge. Should be added to registry.
	require.Equal(t, tcr.Ballot{}, rapp.ballotKeeper.GetBallot(ctx, "Unique registry listing 1"), "Candidate 1 added to registry after losing challenge")
